...
// This next section is for the llhttp shit...

#include "stdio.h"
#include "llhttp.h"
#include "string.h"

int handle_on_message_complete(llhttp_t* someparserthing) {
...
int parse_llhttp(unsigned char* buf, unsigned int len) { // Pass the buffer and length here...
  ...
  */
  ...
  enum llhttp_errno err = llhttp_execute(&llhttp_parser, buf, len);
  if (err == HPE_OK) {
...
int pitchfork_result = 0;


VALUE dangerous_func(VALUE buffer) {

      VALUE parser = rb_class_new_instance(0, NULL, cHttpParser);  // assumes cHttpParser is your class
      // printf("Poopooshit\n");
      // VALUE req = rb_str_new_cstr(buffer);

      // Feed the request string
      //rb_funcall(parser, rb_intern("<<"), 1, buffer);
      //printf("After the shit...\n");
      // Call parse


      VALUE result2 = rb_funcall(parser, rb_intern("clear"), 0);

      VALUE result = rb_funcall(parser, rb_intern("add_parse"), 1, buffer);

      // Inspect result (should be a hash or nil)
      /*
      if (NIL_P(result)) {
          printf("Need more data\n");
      } else {
          // Maybe print keys in result hash
      }
      */
...
VALUE error_func(VALUE exc, VALUE data) {
    // rb_warn("Exception: %s", RSTRING_PTR(rb_obj_as_string(exc)));
    pitchfork_result = 1; // Error here...
    return Qnil;
}
...
int main(int argc, char **argv) {
        ...
        while (__AFL_LOOP(LOOP_COUNT)) {
                ...
                llhttp_result = parse_llhttp(buf, len);

                rb_rescue(dangerous_func, hello_world_str, error_func, Qnil);
                // Now try to parse using llhttp...
                
                // Now do the shit...
                
                if (llhttp_result != pitchfork_result) {
                  printf("LLHTTP result is this: %d\n", llhttp_result);
                  printf("Pitchfork result is this: %d\n", pitchfork_result);
                  //int* x = 0;
                  //*x = 10; // Null pointer write here...
                  abort(); // Just do the shit here...
                }
